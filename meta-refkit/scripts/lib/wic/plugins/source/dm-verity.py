...
class DMVerityPlugin(SourcePlugin):
    ...
    def do_prepare_partition(cls, part, source_params, creator, cr_workdir,
        ...
        if len(rootfs) != 1:
            msger.error("%s shell pattern does not match exactly one rootfs image (missing --label parameter?): %s" % (pattern, rootfs))
        else:
            rootfs = rootfs[0]
        msger.debug("Calculating dm-verity hash for rootfs %s (native %s)." % (rootfs, native_sysroot))
        ...
        if ret or not m:
            msger.error('veritysetup failed: %s' % out)
        else:
            root_hash = m.group(1)
            ...
            try:
                ...
                ret, out = exec_native_cmd("openssl dgst -sha256 -passin '%s' -sign '%s' -out '%s' '%s'" %
                                           ...
                                           native_sysroot)
                if ret:
                    msger.error('openssl signing failed')
                with open(signature, 'rb') as f:
                    header += b'signature=' + base64.standard_b64encode(f.read()) + b'\n'
                if len(header) + 1 >= header_size:
                    msger.error('reserved space for dm-verity header too small')
            ...
            msger.debug("dm-verity data partition %d bytes, hash partition %d bytes, ratio %f." %
