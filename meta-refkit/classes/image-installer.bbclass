...
# "installer image" which can be booted from a removal media to
...
# checks with checkbashisms.pl and checkshell. The "local"
...
INSTALLER_FIND_OUTPUT () {
    ...
    _find_output () (
        ...
        if [ "$root_device" ]; then
            ...
            # Here we are relying on the fact that dependent items appear
            # below the disk they are rooted in.
            local disk=
            exclude=$( lsblk --output KNAME,TYPE --ascii --noheadings | while read kname type; do
                if [ "$type" = "disk" ]; then
                    disk=$kname
                fi
