            ...
            if [ -e "$bootparam_root" ]; then
                ...
                case $? in
                  0) # LUKS volumne found
                    ...
                    if cryptsetup open --type luks "$bootparam_root" "${REFKIT_DEVICE_MAPPER_NAME}" --key-file "$keyfile" --keyfile-offset "$keyfile_offset"; then
                        bootparam_root="/dev/mapper/${REFKIT_DEVICE_MAPPER_NAME}"
                        luks_cleanup
                        return
                    fi
                    luks_cleanup
...
RDEPENDS_${PN} = " \
    cryptsetup \
    ${@ bb.utils.contains('DISTRO_FEATURES', 'tpm1.2', 'trousers tpm-tools strace lsof netbase init-ifupdown', '', d) } \
"
