...
class AlsaTest(oeRuntimeTest):
    def test_rec_play(self):
        # Recording audio
        (status, output) = self.target.run("arecord -d 1 -f cd -D plughw:1,0 /home/rec.wav")
        ...
        (status, output) = self.target.run("cat /proc/asound/card1/pcm0p/sub0/status |grep 'state: RUNNING'")
